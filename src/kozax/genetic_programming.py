...
class GeneticProgramming:
    ...
    def punish_duplicates(self, population: Array, fitness: Array) -> Tuple[Array, Array]:
        ...
        coeffs = jnp.arange(1, flat.shape[-1] + 1, dtype=flat.dtype)
        fingerprints = flat @ coeffs  # shape: (population_size,)

        sorted_idx = jnp.argsort(fingerprints)
        sorted_fp = fingerprints[sorted_idx]

        is_dup = jnp.zeros(population.shape[0], dtype=bool)
        is_dup = is_dup.at[sorted_idx[1:]].set(sorted_fp[1:] == sorted_fp[:-1])

        punished_fitness = jnp.where(is_dup[:, None], self.max_fitness * jnp.ones_like(fitness), fitness)
        return population, punished_fitness
    ...
    def update_pareto_front(self, fitness: Array, population: Array) -> None:
        ...
        try:
            ...
            raise("The shape of the fitness does not match ...")
        ...
        pareto_front_host = np.asarray(_pareto_front)
        pareto_fitness_host = np.asarray(_pareto_fitness)

        valid_mask = np.all(pareto_fitness_host != self.max_fitness, axis=-1)
        ...
        kept_fronts = pareto_front_host[valid_idx]
        kept_fitness = pareto_fitness_host[valid_idx]

        flat = kept_fronts.reshape(kept_fronts.shape[0], -1).astype(np.float32, copy=False)
        _, unique_local = np.unique(
            np.ascontiguousarray(flat).view([('', flat.dtype)] * flat.shape[1]),
            return_index=True,
        )

        final_fronts = jnp.asarray(kept_fronts[unique_local])
        final_fitness = jnp.asarray(kept_fitness[unique_local])

        if self.n_objectives == 1:
            final_fitness = jnp.squeeze(final_fitness, axis=-1)

        self.pareto_front = (final_fitness, final_fronts)
